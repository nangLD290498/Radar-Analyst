...
namespace RadarAnalyst.UIComponent
{
    public partial class GCKTabPage : TabPageCI
    {
        ...
        private void drawGraph(object sender, PaintEventArgs e)
        {
            ...
            //float topY = 50.0F;
            //PointF point1 = new PointF(250F, topY);
            //PointF point2 = new PointF(250F, 500.0F);
            //e.Graphics.DrawLine(pen, point1, point2);

            PointF centerCoordinates = new PointF(pictureBoxWidth / 2, pictureBoxHeight / 2);

            drawLine(e, centerCoordinates, 45);
        }

        private static void drawLine(PaintEventArgs e, PointF centerCoordinates, float angle)
        {
            ...
            float startX = centerCoordinates.X + 250F; // <------------|
            ...
            float length = Math.pow(Math.pow(startX - endX, 2) + Math.pow(startY - endY, 2), 0.5);
            float xChange = length * cos(Math.toRadians(angle));
            float yChange = length * sin(Math.toRadians(angle));

            //g.setColor(Color.GRAY);
            g.FillRectangle(whiteBrush, 0, 0, 1000, 1000); //paint over it

            //g.setColor(Color.BLACK);
            g.DrawLine(pen, new PointF(startX, startY), new PointF(endX + xChange, endY + yChange));

            //e.Graphics.RotateTransform(angle);
            //Color whiteColor = Color.White;
            //// draw x line
            //Pen pen = new Pen(whiteColor, 2);
            //PointF point1 = new PointF(centerCoordinates.X + 250F, centerCoordinates.Y);
            //PointF point2 = new PointF(centerCoordinates.X - 250F, centerCoordinates.Y);
            //e.Graphics.DrawLine(pen, point1, point2);
        }

        private static void drawCurve(PaintEventArgs e, Color lineBlueColor, GCKTabPage _this)
        {
            // draw curve
